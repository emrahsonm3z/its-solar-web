    ...
    Array.from({ length: centralCount }).forEach((_, cenIndex) => {
      ...
      Array.from({ length: inverterCount }).forEach((_, invIndex) => {
        ...
        lastTwoHour =
          cenIndex === invIndex
            ...
            : moment().hour() > START_HOUR + 1
              ? lastOneHour + getRandomArbitrary(0, MAX_HOURLY)
              : 0;
        ...
        mockDatas.push({
          ...
          progress: {
            stringsCount: MAX_STRINGS_COUNT,
            workedStrings: getRandomArbitrary(0, MAX_STRINGS_COUNT)
          },
          status,
          lastOneHour,
          lastTwoHour,
